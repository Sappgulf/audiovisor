...
- **v8.14 ship-readiness pass** — the offline tempo analyser now builds its spectra the way the live AnalyserNode does (Blackman window, 0.9 smoothing, ~60 fps hop) and reports the median of confident locks; before this, a 120 BPM kick/snare/hat loop read as 130 and an 80 BPM one as 143. Apple MusicKit is no longer downloaded on every boot — it loads when you connect Apple Music. A Playwright smoke (`npm run e2e`) walks all 23 modes in both renderers, every theme, drawer panel, dialog and shortcut, and the mobile layout, and runs in CI. `vercel.json` adds immutable caching for hashed assets, a no-cache service worker and baseline security headers. Tests build `dist` themselves, so a clean checkout passes `npm test`.
